...
class Activity(models.Model):
    ...
    def get_should_auto_dismiss(self):
        """Check if activity should be auto-dismissed"""
        if self.auto_dismiss and self.get_age_in_hours() >= self.dismiss_after_hours:
            return True
        return False
    ...
    def create_activity(cls, user, activity_type, title, description,
                        priority='medium', related_property=None, related_booking=None,
    ...
    def get_recent_activities(cls, user, limit=20):
        ...
        ).select_related('related_property', 'related_booking')[:limit]
